...
class RAGManager:
    def __init__(self, data_path="dataset/locomo10_rag.json", chunk_size=500, k=1):
        ...
        print(f"Loading embedding model {EMBEDDING_MODEL_PATH} on {EMBEDDING_DEVICE}...")
        self.embedding_model = SentenceTransformer(EMBEDDING_MODEL_PATH, device=EMBEDDING_DEVICE)
    ...
    def calculate_embedding(self, document):
        """
        修改为使用本地 SentenceTransformer 计算
        """
        embedding = self.embedding_model.encode(document, normalize_embeddings=True)
        return embedding
    ...
    def create_chunks(self, chat_history, chunk_size=500):
        """
        修改为使用 BGE-M3 对应的分词器进行切分
        """
        documents = self.clean_chat_history(chat_history)

        if chunk_size == -1:
            embedding = self.calculate_embedding(documents)
            return [documents], [embedding]

        chunks = []
        
        tokens = self.tokenizer.encode(documents, add_special_tokens=False)
        ...
        # 批量计算 Embedding (通常比逐个计算快)
        # show_progress_bar=False 避免进度条刷屏
        embeddings = self.embedding_model.encode(chunks, normalize_embeddings=True, show_progress_bar=False)
    ...
    def process_all_conversations(self, output_file_path):
        ...
        for key, value in tqdm(list(data.items()), desc="Processing conversations"):
            chat_history = value["conversation"]
            questions = value["question"]
...
if __name__ == "__main__":
    chunk_size = 4096
    num_chunks = 1
    rag = RAGManager(data_path="dataset/locomo1_rag.json", chunk_size=chunk_size, k=num_chunks)
    rag.process_all_conversations(f"results/rag_results_{chunk_size}_k{num_chunks}.json")